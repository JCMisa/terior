...
export default function TabsRedesigned({
  room,
  canEdit,
}: {
  room: RoomType;
  ...
  const src = room.redesignedImageUrl || "/designed.png";
  ...
  return (
    <div className="space-y-4 rounded-xl border border-neutral-200 bg-white p-4 dark:border-neutral-800 dark:bg-neutral-950">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold">Redesigned description</h3>
        {canEdit && (
          <EditPopover
            field="redesignedRoomDescription"
            roomId={room.id}
            defaultValue={room.redesignedRoomDescription ?? ""}
          />
        )}
      </div>

      <p className="text-sm text-neutral-700 dark:text-neutral-300">
        {room.redesignedRoomDescription || "No description provided."}
      </p>

      {/* non-editable image */}
      <div className="relative aspect-video w-full overflow-hidden rounded-lg border border-neutral-200 dark:border-neutral-800">
        <Image src={src} alt="Redesigned room" fill className="object-cover" />
